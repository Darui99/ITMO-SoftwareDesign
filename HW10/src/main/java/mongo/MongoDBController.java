...
public class MongoDBController {
    ...
    public Observable<String> registerUser(int id, String nickname, Currency currency) {
        final MongoCollection<Document> users = client.getDatabase(DATABASE_NAME).getCollection(USERS_COLLECTION);
        final Document document = new Document("id", id).append("nickname", nickname)
                .append("currency", currency.toString());
        return users.find(eq("id", id)).toObservable().isEmpty().flatMap(notFound -> {
            if (notFound) {
                return users.insertOne(document).map(x -> "User " + nickname + " was registered");
            } else {
    ...
    public Observable<String> registerProduct(int id, String name, final Map<Currency, Double> prices) {
        final MongoCollection<Document> products = client.getDatabase(DATABASE_NAME).getCollection(PRODUCTS_COLLECTION);
        ...
        return products.find(eq("id", id)).toObservable().isEmpty().flatMap(notFound -> {
            if (notFound) {
                return products.insertOne(document).map(x -> "Product " + name + " was registered");
            } else {
                return Observable.just("Product with id = " + id + " already exists");
            }
        });
    }

    public Observable<String> showProduct(int productId, int userId) {
        Observable<User> user = client.getDatabase(DATABASE_NAME).getCollection(USERS_COLLECTION)
                .find(eq("id", userId)).toObservable().map(User::new);
        return user.isEmpty().flatMap(notFoundUser -> {
            if (notFoundUser) {
                return Observable.just("Specify id of correct user");
            } else {
                Observable<Product> product = client.getDatabase(DATABASE_NAME).getCollection(PRODUCTS_COLLECTION)
